...
def recursive_find_trainer_class_by_name(trainer_name: str):
    ...
    # >>> unified benchmark: allow the short -tr names (swinunetrv1, 3dunet, ...).
    # '3dunet' is not a legal python identifier, so the short names cannot simply
    # be the class names. Anything that is not a known short name passes through.
    from nnunetv2.unified.config import resolve_trainer_name
    trainer_name = resolve_trainer_name(trainer_name)
    # <<< unified benchmark
