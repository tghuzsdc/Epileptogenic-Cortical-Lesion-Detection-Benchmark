...
def accumulate_cv_results(trained_model_folder,
                          ...
                          overwrite: bool = True,
                          val_folder: str = 'validation'):   # >>> unified benchmark
    """
    ...
    for f in folds:
        expected_validation_folder = join(trained_model_folder, f'fold_{f}', val_folder)
        if not isdir(expected_validation_folder):
            raise RuntimeError(f"{expected_validation_folder} is missing. Train fold {f}, or run the "
                               f"validation that produces {val_folder!r} first.")
