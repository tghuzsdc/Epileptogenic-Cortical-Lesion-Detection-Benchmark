...
def accumulate_crossval_results_entry_point():
    ...
    # >>> unified benchmark
    parser.add_argument('-val_folder', type=str, required=False, default='validation',
                        help="Which per-fold folder to accumulate. 'validation' is the one written at "
                             "the end of training; 'validation_best' is written by --val --val_best.")
    # <<< unified benchmark
    args = parser.parse_args()
    # >>> unified benchmark: accept the short -tr names (swinunetrv1, 3dunet, ...)
    ...
    if args.o is None:
        suffix = '' if args.val_folder == 'validation' else f'_{args.val_folder}'
        merged_output_folder = join(
            trained_model_folder,
            f'crossval_results{suffix}_folds_{folds_tuple_to_string(args.f)}')
    else:
