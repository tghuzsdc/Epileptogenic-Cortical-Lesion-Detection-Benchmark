...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_model_folder(self, model_training_output_dir: str,
        ...
        for i, f in enumerate(use_folds):
            ...
            if i == 0:
                ...
                unified_meta = checkpoint.get('unified')

            parameters.append(checkpoint['network_weights'])

        # >>> unified benchmark: rebuild the second head if the model was trained with one
        from nnunetv2.unified import runtime as _unified_runtime
        head2_file = getattr(self, 'unified_head2_file', None)
        dual_head = bool(unified_meta and unified_meta.get('dual_head'))
        if head2_file is not None and not dual_head:
            print('WARNING: -head2 was given but this model was trained with a single output head. '
                  'Ignoring -head2; every case will use the only head there is.')
            head2_file = None
        if dual_head and head2_file is None:
            print('INFO: this model has two output heads and no -head2 list was given, so every case '
                  'will be predicted with head1. Pass -head2 <file> (or -head2 all) to use head2.')
        _unified_runtime.set_inference_setup(head2_file, dual_head)

        # The sliding window runs the same forward pass as training, so it can overflow
        # fp16 the same way. Follow whatever precision the model was trained with.
        # remember how this predictor was set up, so the output folder can say what
        # produced it and apply_postprocessing can refuse an inconsistent pairing
        self.unified_provenance = {
            'checkpoint': os.path.basename(checkpoint_name),
            'model_folder': model_training_output_dir,
            'folds': [str(f) for f in use_folds],
            'architecture': (unified_meta or {}).get('architecture'),
            'training_set': (unified_meta or {}).get('training_set'),
            'amp_dtype': (unified_meta or {}).get('amp_dtype'),
            'dual_head': dual_head,
        }

        amp = (unified_meta or {}).get('amp_dtype')
        if amp in ('bf16', 'fp32', 'fp16'):
            import torch as _torch
            _dtype = {'bf16': _torch.bfloat16, 'fp32': _torch.float32,
                      'fp16': _torch.float16}[amp]
            if self.device.type == 'cuda':
                try:
                    _torch.set_autocast_dtype('cuda', _dtype)       # torch >= 2.4
                except (AttributeError, TypeError):
                    _torch.set_autocast_gpu_dtype(_dtype)           # older torch
                print(f'INFO: predicting in {amp}, the precision this model was trained with')

        self.unified_dual_head = dual_head
        self.unified_head2_cases = _unified_runtime.get_head2_cases() if dual_head else None
        if unified_meta is not None:
            print(f"INFO: unified model: architecture={unified_meta.get('architecture')}, "
                  f"training set={unified_meta.get('training_set')}, dual_head={dual_head}")
        # <<< unified benchmark
    ...
    # >>> unified benchmark
    def _unified_route_head(self, ofile: Optional[str]):
        """Point a two-head network at the head this case should use.

        Called once per case, before the sliding window runs. Cases listed in the
        ``-head2`` file go through head2, everything else through head1.
        """
        if not getattr(self, 'unified_dual_head', False):
            return
        from nnunetv2.unified.dual_head import DualHeadWrapper
        from nnunetv2.unified.subsets import is_head2

        network = self.network
        if isinstance(network, OptimizedModule):
            network = network._orig_mod
        if not isinstance(network, DualHeadWrapper):
            return

        case = os.path.basename(ofile) if ofile else ''
        head = 1 if is_head2(case, getattr(self, 'unified_head2_cases', None)) else 0
        network.set_head_index(None)
        network.set_active_head(head)
        if self.verbose:
            print(f'  routing {case or "case"} through head{head + 1}')
    # <<< unified benchmark
    ...
    def predict_from_files(self,
        ...
        if output_folder is not None:
            ...
            # >>> unified benchmark: which weights/precision/constraint made these files
            from nnunetv2.unified import cluster_constraint as _cc
            _prov = dict(getattr(self, 'unified_provenance', {}))
            _prov['cluster_constraint'] = _cc.get_mode()
            _prov['cluster_topk'] = _cc.get_topk() if _cc.get_mode() else None
            _prov['head2_file'] = getattr(self, 'unified_head2_file', None)
            save_json(_prov, join(output_folder, 'unified_provenance.json'), sort_keys=False)
            # <<< unified benchmark
        #######################
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                self._unified_route_head(ofile)  # >>> unified benchmark: pick head1/head2 for this case
    ...
    def predict_from_files_sequential(self,
        ...
        for li, of, sps in zip(list_of_lists_or_source_folder, output_filename_truncated, seg_from_prev_stage_files):
            ...
            self._unified_route_head(of)  # >>> unified benchmark: pick head1/head2 for this case
...
def predict_entry_point():
    ...
    # >>> unified benchmark flags
    constraint = parser.add_mutually_exclusive_group()
    constraint.add_argument('-A', dest='cluster_constraint', action='store_const', const='A',
                            help='[OPTIONAL] keep only the top-3 26-connected components, ranked by the 95th '
                                 'percentile of the probabilities inside each component. Applied during inference, '
                                 'on the softmax of each case, not as a post-processing step.')
    constraint.add_argument('-B', dest='cluster_constraint', action='store_const', const='B',
                            help='[OPTIONAL] as -A, but components are ranked by the mean of their 50 '
                                 'highest-probability voxels.')
    constraint.add_argument('-C', dest='cluster_constraint', action='store_const', const='C',
                            help='[OPTIONAL] as -A, but components are ranked by the mean of the probabilities '
                                 'above 0.3 inside the component.')
    parser.set_defaults(cluster_constraint=None)
    parser.add_argument('-topk', type=int, required=False, default=3,
                        help='[OPTIONAL] how many components -A/-B/-C keep. Default: 3')
    parser.add_argument('-head2', type=str, required=False, default=None,
                        help='[OPTIONAL] path to a txt file listing, one per line, the cases to run through the '
                             'second output head. Everything not listed uses head1. Pass the literal value "all" '
                             'to route every case through head2 (M4 inference).')
    # <<< unified benchmark flags
    ...
    # >>> unified benchmark: resolve the short -tr names and arm the inference options
    from nnunetv2.unified.config import resolve_trainer_name
    from nnunetv2.unified import cluster_constraint as _cc
    args.tr = resolve_trainer_name(args.tr)
    if args.head2 is not None and args.head2.strip().lower() != 'all' and not isfile(args.head2):
        raise FileNotFoundError(f'-head2 file not found: {args.head2}')
    _cc.set_mode(args.cluster_constraint, args.topk)
    if args.cluster_constraint is not None:
        print(f'Cluster constraint -{args.cluster_constraint} active: keeping the top {args.topk} '
              f'26-connected components per foreground label.')
    # <<< unified benchmark
    ...
    predictor = nnUNetPredictor(tile_step_size=args.step_size,
                                ...
                                verbose_preprocessing=args.verbose,
                                allow_tqdm=not args.disable_progress_bar)
    # >>> unified benchmark: hand the head2 list to the predictor before loading weights,
    # so that the network is built with two heads when the checkpoint says it has two
    predictor.unified_head2_file = args.head2
    # <<< unified benchmark
