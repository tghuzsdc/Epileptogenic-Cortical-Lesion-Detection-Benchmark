...
def convert_predicted_logits_to_segmentation_with_correct_shape(predicted_logits: Union[torch.Tensor, np.ndarray],
    ...
    # >>> unified benchmark: -A/-B/-C need the probabilities, so the fast argmax-only
    # path is skipped whenever a cluster constraint is armed
    from nnunetv2.unified import cluster_constraint as _cluster_constraint
    _constraint_mode = _cluster_constraint.get_mode()
    _need_probabilities = return_probabilities or _constraint_mode is not None
    # <<< unified benchmark

    # return value of resampling_fn_probabilities can be ndarray or Tensor but that does not matter because
    # apply_inference_nonlin will convert to torch
    if not _need_probabilities:
    ...
    # >>> unified benchmark: top-k 26-connected-component constraint, applied on the
    # probabilities of this case while they are still in memory (inference, not
    # post-processing). Runs at the original voxel grid, before cropping is reverted.
    if _constraint_mode is not None:
        segmentation = _cluster_constraint.maybe_apply(segmentation, predicted_probabilities,
