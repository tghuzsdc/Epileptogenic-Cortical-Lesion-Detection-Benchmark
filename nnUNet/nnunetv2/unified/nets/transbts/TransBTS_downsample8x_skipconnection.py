...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        # [unified] the official code hard-codes BraTS's 4 input channels and 4 classes here,
        # which ignores the num_channels/num_classes arguments it was constructed with
...
class BTS(TransformerBTS):
    def __init__(
        ...
        # [unified] number of classes was hard-coded to BraTS's 4
    ...
    def decode(self, x1_1, x2_1, x3_1, x, intmd_x, intmd_layers=[1, 2, 3, 4]):
        ...
        y = self.endconv(y2)      # (1, num_classes, 128, 128, 128)
        # [unified] the official code applies softmax here; nnU-Net's loss and its
        # inference non-linearity both expect raw logits, so it is left off
        # y = self.Softmax(y)
        return y
